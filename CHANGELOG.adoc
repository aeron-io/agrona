...
* *[Breaking]* Remove `agrona-agent` module, i.e. `BufferAlignmentAgent` is no longer provided.

=== Changelog

* **[Java]** Unmap null values returned by `Int2ObjectHashMap.EntryIterator.setValue`. (https://github.com/aeron-io/agrona/pull/374[#374])
* **[Java]** Unmap null values in `BiInt2ObjectMap.toString`. (https://github.com/aeron-io/agrona/pull/373[#373])
* **[Java]** Unmap null values returned by `BiInt2ObjectMap.remove`. (https://github.com/aeron-io/agrona/pull/372[#372])
* **[Java]** Reject non-ASCII digit look-alikes in AsciiEncoding SWAR fast paths. (https://github.com/aeron-io/agrona/pull/367[#367])
* **[Java]** Eliminate more `Arrays.fill` calls when value is zero.
* **[Java]** Refactor collection rehashing to eliminate redundant array filling. (https://github.com/aeron-io/agrona/pull/368[#368])
* **[Java]** Fix `BiInt2NullableObjectMap#forEach` leaking `NullReference`. (https://github.com/aeron-io/agrona/pull/371[#371])
* **[Java]** Fix `ExpandableRingBuffer#append` bug when there is not enough contiguous space left and resize fails.
(https://github.com/aeron-io/agrona/pull/370[#370])
* **[Java]** Upgrade to `Gradle` 9.7.1.
* **[Java]** Upgrade to `ByteBuddy` 1.18.14.
* **[Java]** Upgrade to `Guava TestLib` 33.7.1-jre.
* **[Java]** Upgrade to `Versions` 0.64.0.
...
* **[Java]** Reject `missingValue` in `Int2IntHashMap.replace`. (https://github.com/aeron-io/agrona/pull/364[#364])
* **[Java]** Mask high bytes to Latin-1 in `AsciiSequenceView.charAt`. (https://github.com/aeron-io/agrona/pull/365[#365])
* **[Java]** Reject non-ASCII digit look-alikes in `AsciiEncoding` parsing. (https://github.com/aeron-io/agrona/pull/366[#366])
* **[Java]** Support graceful shutdown of agents. Add an `isExpected` flag to `AgentTerminationException`. Don't log an
expected `AgentTerminationException` with the error handler within the `AgentRunner` and `AgentInvoker`.
...
* *[Breaking]* `org.agrona.concurrent.AgentRunner.close()` methods are no longer interruptible.
+
Previously when a thread was interrupted while calling `org.agrona.concurrent.AgentRunner.close()` or
`org.agrona.concurrent.AgentRunner.close(int, java.util.function.Consumer<java.lang.Thread>)`
it would immediately exit that method after interrupting agent thread. Now the thread will remain in the waiting loop
until agent thread has terminated.
+
[IMPORTANT]
====
Make sure that the `org.agrona.concurrent.Agent.doWork` eventually terminates, i.e. do not use blocking operations or handle thread
